...
total_terminated = int(
    total_employees * termination_percentage
)
...
for year, weight in termination_year_weights.items():
    ...
    termination_dates.extend(
        [year] * num_terminations
    )
...
terminated_indices = df.index[
    :total_terminated
]


for i, year in enumerate(
    termination_dates[:total_terminated]
):

    df.at[
        terminated_indices[i],
        'termdate'
    ] = (
...
# Make sure termination is at least
...
print(
    f"Total Terminated: {df['termdate'].notna().sum()}"
)
...
print(
    "\nHumanResources.csv created successfully!"
)
